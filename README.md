...
**YOLO Web UI** is a user-friendly web interface for YOLO object detection based on Gradio. This project provides a production-ready web interface that allows users to perform object detection through a simple browser interface.
...
- 📹 **Webcam Detection** - Real-time detection via webcam
...
│
└── YOLO/                       # Ultralytics YOLO Library (dependency)
    └── ...                     # YOLO library files
```
...
- Optimized for performance (20 FPS)
...
**YOLO Web UI** 是一个基于Gradio的YOLO目标检测Web界面。本项目提供了一个生产就绪的网页界面，让用户可以通过简单的浏览器界面进行目标检测。
