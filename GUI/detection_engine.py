...
class DetectionEngine:
    ...
    def detect_image(self, image: np.ndarray, conf_threshold: float = 0.25) -> DetectionResult:
        """检测单张图片"""
        if image is None:
        ...
        if results[0].boxes is not None and len(results[0].boxes) > 0:
            ...
            for i, (conf, cls, box) in enumerate(zip(confidences, classes, xyxy)):
                class_name = self.class_names[cls] if cls < len(self.class_names) else f"类别{cls}"
    ...
    def process_video(self, video_path: str, conf_threshold: float = 0.25, 
        ...
        try:
            while True:
                ...
                yield frame_count, total_frames, result_frame, end_time - start_time, object_count

        finally:
            cap.release()

    def process_webcam_frame(self, frame: np.ndarray, conf_threshold: float = 0.25) -> np.ndarray:
        """处理摄像头帧"""
        if frame is None:
            return None
